...
waitOpen = codecs.open("wait1.json","r","utf-8")
...
with open("temp1.json", "r", encoding="utf_8_sig") as f:
    ...
    anu.update(settings)
    settings = anu
with open("wait1.json", "r", encoding="utf_8_sig") as f:
    itu = json.loads(f.read())
    itu.update(wait)
    wait = itu
#=====================================================================
...
def mentions(to, text="", mids=[]):
    ...
    client.sendMessage(to, textx, {'AGENT_NAME':'LINE OFFICIAL', 'AGENT_LINK': 'line://ti/p/~{}'.format(client.getProfile().userid), 'AGENT_ICON': "http://dl.profile.line-cdn.net/" + client.getContact("u085311ecd9e3e3d74ae4c9f5437cbcb5").picturePath, 'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
#=====================================================================
#=====================================================================
#=====================================================================
def load():
...
def backupData():
    try:
        ...
        json.dump(backup, f, sort_keys=True, indent=4, ensure_ascii=False)
        backup = wait
        f = codecs.open('wait1.json','w','utf-8')
        json.dump(backup, f, sort_keys=True, indent=4, ensure_ascii=False)
        backup = stickers1
...
#                    if client.getCompactGroup(op.param1).preventedJoinByTicket == False:
#                        client.reissueGroupTicket(op.param1)
#                        X = client.getCompactGroup(op.param1)
#                        X.preventedJoinByTicket = True
#                        Ticket = client.reissueGroupTicket(op.param1)
#                        haha.acceptGroupInvitationByTicket(op.param1,Ticket)
#                        hihi.acceptGroupInvitationByTicket(op.param1,Ticket)
#                        ckck.acceptGroupInvitationByTicket(op.param1,Ticket)
#                        random.choice(ABC).kickoutFromGroup(op.param1,[op.param2])
#                        haha.updateGroup(X)
 #                       haha.leaveGroup(op.param1)
#                        hihi.leaveGroup(op.param1)
#                        ckck.leaveGroup(op.param1)
        if op.type == 11:
                ...
                if msg.contentType == 0:
                    if "/ti/g/" in text.lower():
                        ...
                        if msg._from in ["u083bdc9ca7257b41cd4d1de933a5ad8e"] and msg.toType == 2:
                            ...
                            client.sendMention(to, "LU SIAPA GOBLOK @!??","",[msg._from])
...
#                        sendStickers(to, sver, spkg, sid)
                ...
                for sticker in stickers1:
                    if text.lower() == sticker:
                        ...
                        data = {
                            "type": "template",
                            "altText": "           [̲̅T][̲̅M] [̲̅S][̲̅A][̲̅M][̲̅U][̲̅R][̲̅A][̲̅I]            \n                   ˢᴱᴸᶠᴮᴼᵀ ᵀᴴᴬᴵᴸᴬᴺᴰ\n              𝕊𝔼𝕃𝔽𝔹𝕆𝕋 𝕋ℍ𝔸𝕀𝕃𝔸ℕ𝔻",
                            "baseSize": {
                                "height": 1040,
                                "width": 1040
                            },
                      ...
                      for cmd in cmd.split(" & "):
								        ...
								        "contents": [
								          {
								            ...
								            "contents": [
								              ...
								              {
								                "type": "text",
								                "text": "หอนาฬิกา เชียงราย  ซอย วัดเกศ 57100",
								                "wrap": true,
								                "color": "#666666",
								                "size": "sm",
								                "flex": 5,
                        ...
                        if cmd.startswith('cek mention ') or cmd == 'mentionme':cekmentions(to,wait,cmd)
                            ...
                            try:
                                ...
                                ret_ += "\nMsg send : {}".format(str(peler["sendcount"]))
                                ret_ += "\nMsg received : {}".format(str(peler["receivercount"]))
                                ret_ += "\n\n< About Bots >\n"
                                ret_ += "\nType : Selfbot"
                                ret_ += "\nVersion : V.07\n"
                                ret_ += "\nEditor :\n"
                             #   ret_ += "@!                  "
                                mentions(to, str(ret_))
                            except Exception as e:
                                client.generateReplyMessage(msg.id)
                        ...
                        elif cmd == "ลบแชท" and sender == clientMID:
...
#                                client.sendMessage(msg.to,"• PROTECTION •\n\n- Protect Url :\n"+ma+"\n- Protect Kick :\n"+mb+"\n- Protect Join :\n"+md+"\n- Protect Invite :\n"+me+"\nTotal「%s」Protect" %(str(len(protectqr)+len(protectkick)+len(protectjoin)+len(protectinvite))))
                        ...
                        if cmd == "speed":
                            ...
                            sendTemplate(to,data)
                        elif cmd == "รีบูส" and sender == clientMID:
                            try:
...
#=====================================================================
                        ...
                        elif cmd.startswith("ลบคำห้ามพิม "):
                            wunban = cmd.split()[1:]
                            wunban = " ".join(wunban)
...
#==========================================
                        ...
                        elif cmd == "ลบรัน" and sender == clientMID:
                        ...
                        elif cmd == "ยกเชิญ" and sender == clientMID:
                            if msg.toType == 2:
                                group = client.getGroup(to)
...
#                              \\ COMMAND PROFILE //
                        elif cmd == "บล็อค" and sender == clientMID:
                            blockedlist = client.getBlockedContactIds()
...
#=====================================================================
                            ...
                            if len(count) == 1:
                                ...
                                try:
                                    ...
                                    client.sendMessage(msg.to," "+str(e))
                        elif cmd.startswith("ไอดีไลน "):
                            a = removeCmd("ไอดีไลน", text)
        ...
        if op.type == 55:
            ...
            if op.param1 in settings["getReader"] and op.param2 not in settings["getReader"][op.param1]:
                ...
                if "@!" in settings["readerPesan"]:
                    ...
                    sendMention(op.param1, op.param2, "Gw", settings["readerPesan"])
